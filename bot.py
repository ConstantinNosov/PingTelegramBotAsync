...
from pingtester_lib import AsyncPingTester
...
async def main():
    ...
    # Запуск задачи отправки уведомления о работоспособности
    asyncio.create_task(daily_notification(bot))
    while True:
        await ip_test(bot)
        # Проверка каждые 5 минут
        await asyncio.sleep(5)  # Изменено с 5 секунд для реалистичности
